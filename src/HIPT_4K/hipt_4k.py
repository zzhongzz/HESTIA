...
class HIPT_4K(torch.nn.Module):
    ...
    def forward_all256(self, x):
        ...
        for mini_bs in range(
            ...
            fea_all256 = self.model256.forward_all(minibatch_256)
            fea_cls256 = fea_all256[:, 0] # shape (B, C), where B = (1*w_256*h_256), C = 384
            fea_sub256 = fea_all256[:, 1:] # shape (B, 256, C)
    ...
    def forward_all4k(self, features_cls256):
        B, C, w_256, h_256 = features_cls256.shape
        ...
        features_cls4k = features_all4k[
            :, 0
        ]  # 5. [B x 192], where 192 == dim of ViT-4K [ClS] token.
        features_sub4k = features_all4k[:, 1:]
        features_sub4k = features_sub4k.reshape(B, w_256, h_256, 192).permute(
            0, 3, 1, 2
        )
